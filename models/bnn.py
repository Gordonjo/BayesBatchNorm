...
class bnn(object):
    ...
    def train(self, Data, n_epochs, batchsize, lr, eval_samps=None, binarize=False, logging=False, verbose=1):
	...
	y_ = tf.reduce_mean(self.predict(x, self.wSamps, training=True), axis=0)
	...
	self.n = data.N
	self.n_train = data.TRAIN_SIZE
	self.n_test = data.TEST_SIZE
	if eval_samps == None:
	...
	acc_train = sess.run(self.compute_acc(self.x, self.y), {self.x:data.data['x_train'], self.y:data.data['y_train']})
	acc_test = sess.run(self.compute_acc(self.x, self.y), {self.x:data.data['x_test'], self.y:data.data['y_test']})
	kl, ll = sess.run([self.kl_term, self.l], {self.x:x, self.y:y})
	self.train_acc[epoch-1], self.test_acc[epoch-1] = acc_train, acc_test 
	print('Epoch {}: Training: {:5.3f}, Testing: {:5.3f}, KL: {:5.3f}, Data: {:5.3f}'.format(epoch, acc_train, acc_test, kl/self.n_train, ll))
