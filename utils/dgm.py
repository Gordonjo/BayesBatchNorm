...
def initGaussBNN(n_in, n_hid, n_out, vname, initVar=-5, bn=None):
    ...
    weights['Wmean_mean'] = tf.get_variable(shape=[n_hid[-1], n_out], name=vname+'Wout_mean', initializer=xavier_initializer())
    weights['Wmean_logvar'] = tf.Variable(tf.fill([n_hid[-1], n_out], initVar), name=vname+'Wout_logvar')
    weights['bmean_mean'] = tf.Variable(tf.zeros([n_out]) + 1e-1, name=vname+'bout_mean')
    weights['bmean_logvar'] = tf.Variable(tf.fill([n_out], value=initVar), name=vname+'bout_logvar')
    weights['Wvar_mean'] = tf.get_variable(shape=[n_hid[-1], n_out], name=vname+'Wout_mean', initializer=xavier_initializer())
    weights['Wvar_logvar'] = tf.Variable(tf.fill([n_hid[-1], n_out], initVar), name=vname+'Wout_logvar')
    weights['bvar_mean'] = tf.Variable(tf.zeros([n_out]) + 1e-1, name=vname+'bout_mean')
    weights['bvar_logvar'] = tf.Variable(tf.fill([n_out], value=initVar), name=vname+'bout_logvar')
    return weights
...
def sampleGaussBNN(weights, n_hid):
    ...
    wTilde['Wmean'], wTilde['bmean'] = tf.squeeze(sampleNormal(meanW, logvarW)), tf.squeeze(sampleNormal(meanB, logvarB))
    meanW, meanB = weights['Wvar_mean'], weights['bvar_mean']
    logvarW, logvarB = weights['Wvar_logvar'], weights['bvar_logvar']
    wTilde['Wvar'], wTilde['bvar'] = tf.squeeze(sampleNormal(meanW, logvarW)), tf.squeeze(sampleNormal(meanB, logvarB))
    return wTilde
